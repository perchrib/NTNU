...
class Draw(tk.Tk):
	def __init__(self,dimension):
		...
		self.random_button = tk.Button(self.topframe, text="RANDOM",width=10,height=5)
		"""PACK"""
		self.topframe.pack(side='top') 
		self.random_button.pack(padx=2,side='right')
		...
		self.random_button.bind('<Button-1>',self.random)
	...
	def ai(self,event):
		...
		while not self.board.has_lost():
			#ai.state = self.board
			moves = get_move(self.board.board)
			move = argmax(moves)
			moves[move] = -1
			self.board.move(move+1)
			while not self.board.valid_move:
				move = argmax(moves)
				moves[move] = -1
				self.board.move(move+1)
			new_tile = Tile(None)
	...
	def samples(self):
		random = []
		ai = []
		print("GENERATES RANDOM......")
		for x in range(50):
			self.random(None)
			random.append(self.board.get_highest_tile())
		print("GENREATES AI......")
		for x in range(50):
			self.ai(None)
			ai.append(self.board.get_highest_tile())
		return random,ai
